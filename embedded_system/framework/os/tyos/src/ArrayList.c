...
void ArrayList_Insert(arraylist *al, uint16_t idx)
{
    ...
    {
        uint8_t *new_addr = Malloc(al->Heap, (al->MaxNum + al->MinMalloc_Num) * al->Member_Size);
        ...
        al->MaxNum = Malloc_Size(al->Heap, new_addr) / al->Member_Size;
        Free(al->Heap, al->Members);
        al->Members = new_addr;
    }
    else
...
void ArrayList_Remove(arraylist *al, uint16_t idx)
{
    ...
    if (Malloc_Size(al->Heap, al->Members) - (al->Member_Num - 1) * al->Member_Size >= Memory_Block_Size(al->Heap))
    {
        uint8_t *new_addr = Malloc(al->Heap, (al->Member_Num - 1) * al->Member_Size);
        memcpy(new_addr, al->Members, idx * al->Member_Size);
